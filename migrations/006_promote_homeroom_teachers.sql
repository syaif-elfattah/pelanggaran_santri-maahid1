-- Fase 2 - Naik Kelas: wali kelas ikut ke kelas tujuan secara default.
-- Jalankan di SQL Editor Supabase, SETELAH migrations/005.

drop function if exists public.promote_academic_year(text, date, date, jsonb);
...
create or replace function public.promote_academic_year(
  ...
  p_new_year_end date,
  p_mappings jsonb,
  p_copy_homeroom_teachers boolean default true
) returns uuid
language plpgsql
as $$
declare
...
begin
  ...
  loop
    ...
    if v_action = 'lulus' then
      ...
      -- Wali kelas ikut ke kelas tujuan secara default (bisa diubah manual
      -- lewat Manajemen Kelas kalau tahun ini gantinya beda orang).
      if p_copy_homeroom_teachers then
        insert into public.homeroom_teachers (class_id, academic_year_id, name, phone)
        select v_to_class_id, v_new_year_id, ht.name, ht.phone
        from public.homeroom_teachers ht
        where ht.class_id = v_from_class_id
          and ht.academic_year_id = v_old_year_id
        on conflict (class_id, academic_year_id) do nothing;
      end if;
    end if;
  end loop;
...
grant execute on function public.promote_academic_year(text, date, date, jsonb, boolean) to service_role;
